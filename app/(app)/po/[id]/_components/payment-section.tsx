...
import { CreditCard, MoreVertical, ExternalLink, Trash2, CheckCircle2 } from "lucide-react";
...
export function PaymentSection({
  ...
  const status: "unpaid" | "partial" | "paid" | "overpaid" =
    paidAmount === 0 ? "unpaid"
      : paidAmount < poTotal ? "partial"
        : paidAmount === poTotal ? "paid" : "overpaid";
  ...
  function handleVoid(p: PoPayment) {
    if (typeof window === "undefined") return;
    const reason = window.prompt(
      `ยืนยันลบ payment ฿${fmtBaht(p.amount)} (${p.card_display})?\nเหตุผล:`,
      "",
    );
    if (reason === null) return;
    start(async () => {
      const r = await voidPaymentAction(p.id, reason);
      if (r.ok) { toast.success("ลบ payment แล้ว"); router.refresh(); }
      else toast.error(r.error ?? "ลบไม่สำเร็จ");
    });
  }
  ...
  function handleUnmark(p: PoPayment) {
    if (typeof window !== "undefined" && !window.confirm("Undo mark reimbursed?")) return;
    start(async () => {
      const r = await unmarkReimbursedAction({ paymentIds: [p.id] });
      if (r.ok) { toast.success("Undo แล้ว"); router.refresh(); }
      ...
      <CardContent className="p-5">
        <div className="flex items-baseline justify-between gap-2 flex-wrap mb-3">
          <h3 className="text-sm font-bold text-slate-900 inline-flex items-center gap-1.5">
            <CreditCard className="size-4 text-brand-600" /> การชำระเงิน
          ...
          <div className="mt-4">
            ...
            <div className="space-y-2">
              {payments.map((p) => (
                <div
                  key={p.id}
                  className="border border-slate-200 rounded-lg p-3 bg-white hover:bg-slate-50/50 transition-colors"
                >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {p.reimbursed ? (
                        <span className="text-[10px] font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full inline-flex items-center gap-1">
                          <CheckCircle2 className="size-3" /> เบิกแล้ว
                        </span>
                      ) : (
                      ...
                      {(isAdmin || (isCreator && !p.reimbursed)) && (
                        <div className="relative">
                          ...
                          {menuFor === p.id && (
                            <div
                              className="absolute right-0 top-6 z-10 bg-white border border-slate-200 rounded-lg shadow-lg py-1 min-w-[180px] text-sm"
                              onClick={() => setMenuFor(null)}
                            >
                              {isAdmin && !p.reimbursed && (
                                <button
                                  className="w-full text-left px-3 py-1.5 hover:bg-emerald-50 text-emerald-700 flex items-center gap-2"
                                  onClick={() => { setMarkingFor(p); setMarkRef(""); }}
                                >
                                  <CheckCircle2 className="size-3.5" /> Mark เบิกแล้ว
                                </button>
                              )}
                              {isAdmin && p.reimbursed && (
                                <button
                              ...
                              {isAdmin && (
                                ...
                                >
                                  <Trash2 className="size-3.5" /> ลบ payment
                                </button>
                              )}
                            </div>
                          )}
                  ...
                  <div className="flex items-center gap-3 mt-1.5 flex-wrap text-xs text-slate-500">
                    {p.slip_url && (
                      ...
                      >
                        📎 ดูสลิป <ExternalLink className="size-3" />
                      </a>
                    )}
                    {p.payment_group_id && (
          ...
          >
            ...
            >
              ...
              <div className="flex justify-end gap-2 mt-4">
                <Button variant="outline" onClick={() => setMarkingFor(null)} disabled={pending}>
                  ยกเลิก
                </Button>
                <Button onClick={handleMark} disabled={pending}>
