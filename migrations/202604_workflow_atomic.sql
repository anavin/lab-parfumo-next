...
-- แก้ race condition:
--   1) addDeliveryAction: 2 user กดรับพร้อมกัน → delivery_no ชนกัน
--   2) Stock increment: read-modify-write race → ตัวเลข stock ผิด
-- ============================================================
...
-- 2. Atomic stock increment (signed: +qty รับของ, -qty rollback)
...
-- 3. (optional) ฟังก์ชั่นสะดวก: หา delivery_no ถัดไปแบบ atomic
--    ใช้ advisory lock บน po_id เพื่อ serialize การ INSERT
CREATE OR REPLACE FUNCTION next_po_delivery_no(p_po_id UUID)
RETURNS INT AS $$
DECLARE
  next_no INT;
BEGIN
  -- Advisory lock by hashing UUID (transaction-scoped — auto-release)
